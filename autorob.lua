...
-- ===== NEUE REMOTES (GlobalEvents) =====
local GlobalEvents = ReplicatedStorage:WaitForChild("shared/modules/globalNetworking@GlobalEvents")

local RemoteEvents = {
    rob        = GlobalEvents:WaitForChild("rob"),               -- bleibt so (falls abweichend, hier anpassen)
    sell       = GlobalEvents:WaitForChild("sellItem"),
    equip      = GlobalEvents:WaitForChild("equipTool"),
    buy        = GlobalEvents:WaitForChild("buyItem"),
    bomb       = GlobalEvents:WaitForChild("detonateBombs"),     -- KORREKTUR!
    OpenPhone  = GlobalEvents:WaitForChild("equipTool"),
    ClosePhone = GlobalEvents:WaitForChild("unequipTool"),
    hand       = GlobalEvents:WaitForChild("unequipTool"),
}
...
local Config = {
    ...
    collectSpeedDivider  = 28,
}

local LOW_HEALTH_THRESHOLD = 35
local POLICE_CHECK_RANGE = 25
...
local WebhookGroup = Window:MakeTabGroup({ Name = "Webhook" })
...
local tabs = {
    ...
    Webhook = WebhookGroup:MakeTab({ Name = "Webhook",  Icon = "rbxassetid://4483345998" }),
}
...
local function saveConfig()
    local config = {
        ...
        collectSpeedDivider    = Config.collectSpeedDivider,
    }
...
local function loadConfig()
    ...
    if isfile and isfile(configFileName) then
        ...
        if success and data then
            ...
            if data.collectSpeedDivider ~= nil then Config.collectSpeedDivider = data.collectSpeedDivider end
        end
    end
end
...
-- ===== Info-Tab =====
tabs.Info:AddSection({ Name = "Warning" })
tabs.Info:AddParagraph("by Night Tools ",
    "You may get kicked due to high graphics or performance issues. " ..
    "If you experience errors, lower your graphics settings before running again."
)
tabs.Info:AddParagraph("Info",
    "Developer: Maxi\nDiscord: discord.gg/jsankjhV6Z"
)
...
tabs.AutoRob:AddSlider({
    ...
    Min       = 15,
    Max       = 70,
    Default   = Config.collectSpeedDivider,
    Color     = Color3.fromRGB(255, 239, 149),
    Increment = 1,
    ValueName = "speed",
    Callback  = function(v)
        Config.collectSpeedDivider = v
        saveConfig()
    end,
})
...
-- ===== Webhook-Tab =====
tabs.Webhook:AddSection({ Name = "Webhook Configuration" })
...
tabs.Webhook:AddButton({
    ...
    Callback = function()
        if WebhookUrl == "" then
            sendNotification("Error", "Please enter a Webhook URL first.")
            return
        end
        local data = {
            content = "Webhook test from Night System.",
            embeds = {{
                title = "Test",
        ...
        if requestFunc then
            pcall(function()
                ...
                sendNotification("Success", "Webhook test sent.")
            end)
        else
            sendNotification("Error", "No request function available.")
        end
    end
})

-- ===== Restlicher Script-Code (unverändert, aber mit neuen Remote-Namen) =====
...
local function isPoliceNearby()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= Player and plr.Team == policeTeam and plr.Character then
            ...
            if pHRP and HumanoidRootPart and (pHRP.Position - HumanoidRootPart.Position).Magnitude <= POLICE_CHECK_RANGE then
                return true
            end
        end
    end
...
local function isPlayerHurt()
    ...
    return humanoid and humanoid.Health <= LOW_HEALTH_THRESHOLD
end
...
local function teleportPlayer(destination)
    ...
    if State.fastTeleportToggle and dist < 25 then
        char:SetPrimaryPartCFrame(CFrame.new(destination))
    else
        local tweenDuration = math.max(0.5, dist / Config.collectSpeedDivider)
...
local function HealthCheck()
    local function watchChar(char)
        ...
        humanoid.HealthChanged:Connect(function(health)
            ...
            if health > 0 and health <= LOW_HEALTH_THRESHOLD then
                HealthTripwire = true
...
local function runMainRobberySequence()    
    ...
    if musikPart.Rotation == Vector3.new(180, 0, 180) then
        ...
        SpawnBomb()
        task.wait(0.25)
        RemoteEvents.bomb:FireServer()   -- jetzt detonateBombs
